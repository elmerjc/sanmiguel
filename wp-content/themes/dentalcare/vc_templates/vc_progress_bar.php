<?php
$output = $title = $values = $units = $bgcolor = $custombgcolor = $options = $el_class = $progress_bar_style = $progress_bar_class = '';
...
if( $progress_bar_style != '' ) {
	$progress_bar_class = " " . $progress_bar_style;
}
...
$output = '<div class="' . $css_class . ''. $progress_bar_class .'">';
...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<span class="vc_bar' . $bar_options . '" data-percentage-value="' . ( $percentage_value ) . '" data-value="' . $line['value'] . '"' . $line['bgcolor'] . '>'. ( ( $progress_bar_style == 'style_2' ) ? '<span class="vc_bar_value-marker">'. $percentage_value .'%</span>' : '' ) .'</span>';
	$output .= '</div>';
}
