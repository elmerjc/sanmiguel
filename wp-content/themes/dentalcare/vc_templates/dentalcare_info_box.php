...
if( !empty( $vc_image_size ) ) {
	...
	$image_size = 'dentalcare_image_350x204_croped';
}

if( $style == 'style_3' ){
	$image_size = 'dentalcare_image_350x250_croped';
}
...
<div class="info_box<?php echo esc_attr( $css_class ); ?>">
	...
	<?php if( $style == 'style_6' ) : ?>
		...
		<div class="info_box_text">
			<div class="title">
				<div class="icon">
					<i class="<?php echo esc_attr( $title_icon ); ?>"<?php echo esc_attr($icon_style); ?>></i>
				</div>
			...
			if ( $link['url'] ) {
				if ( ! $link['title'] ) {
					$link['title'] = esc_html__( 'Read more', 'dentalcare' );
				}
				if ( ! $link['target'] ) {
					$link['target'] = '_self';
				}
				if( $icon ){
					$link['title'] = '<span>' . esc_html( $link['title'] ) . '</span>' . '<i class=" ' . esc_attr( $icon ) . ' dentalcare_icon"></i>';
				}
	...
	<?php else : ?>
		...
		<?php if( $style == 'style_4' ) : ?>
			<?php if( $image ): ?>
				...
				?>
				<div class="info_box_image"><?php echo esc_attr($image_data['thumbnail']); ?></div>
			<?php endif; ?>
		<?php else : ?>
			<?php if( empty( $vc_image_size ) && $image && $thumbnail = wp_get_attachment_image( $image, $image_size ) ){ ?>
				<div class="info_box_image"><?php echo esc_attr($thumbnail); ?></div>
			<?php } elseif( $image && !empty( $vc_image_size ) ) { ?>
				...
				?>
				<div class="info_box_image"><?php echo esc_attr($vc_image_data['thumbnail']); ?></div>
			<?php } ?>
		<?php endif; ?>
